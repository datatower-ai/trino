...
@AggregationFunction("retention_lost_date_sim_stat_formula_quota_array_agg_new")
public final class RetentionLostDateSimStatFormulaQuotaCollectAggFunctionNew
{
    private RetentionLostDateSimStatFormulaQuotaCollectAggFunctionNew()
    {
    }
    ...
    public static void input(@AggregationState RetentionLostSimStatFormulaQuotaState state,
            ...
            @SqlType("array(array(bigint))") Block quotaParamBlock,
            @SqlType("array(varchar)") Block paramBlock)
    {
        ArrayType quotaParamType = new ArrayType(new ArrayType(BigintType.BIGINT));
        Block statTypeBlock = quotaParamType.getObject(quotaParamBlock, 0);
        Block quotaTypeBlock = quotaParamType.getObject(quotaParamBlock, 1);
        Block quotaDayBlock = quotaParamType.getObject(quotaParamBlock, 2);
        ...
        DateTimeField dateTimeField = ChronologyUtil.getRetentionTimestampField(ISOChronology.getInstanceUTC(), VarcharType.VARCHAR.getSlice(paramBlock, 1));
        ...
        Slice retentionNumStr = VarcharType.VARCHAR.getSlice(paramBlock, 0);
        ...
        for (int k = 0; k < quotaTypeNum; ++k) {
            long quotaTypeCode = BigintType.BIGINT.getLong(quotaTypeBlock, k);
            quotaTypeArr[k] = quotaTypeCode;
        }
        ...
        RetentionLostSimStatFormulaQuota retentionLostSimStatFormulaQuota = new RetentionLostSimStatFormulaQuota(initSize, statTypes, table, VarcharType.VARCHAR.getSlice(paramBlock, 2), quotaTypeArr, quotaIndexArr);
