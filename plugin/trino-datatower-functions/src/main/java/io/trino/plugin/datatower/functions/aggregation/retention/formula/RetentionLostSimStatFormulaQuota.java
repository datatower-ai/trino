 ...
 * @author liulin
 */
public class RetentionLostSimStatFormulaQuota
{
    private static final long INSTANCE_SIZE = instanceSize(RetentionLostSimStatFormulaQuota.class);
    ...
    private final long[] quotaType;
    private final long[] quotaIndex;

    RetentionLostSimStatFormulaQuota(int rowCount, long[] statTypes, RetentionLostSimStatFormulaItem[] table, Slice formula, long[] quotaType, long[] quotaIndex)
    {
    ...
    static RetentionLostSimStatFormulaQuota merge(RetentionLostSimStatFormulaQuota left, RetentionLostSimStatFormulaQuota right)
    {
        ...
        if (rightRowCount == 0) {
            ...
            while (true) {
                ...
                return new RetentionLostSimStatFormulaQuota(i, left.getStatTypes(), table, left.getFormula(), left.quotaType, left.quotaIndex);
            }
        }
    }
    ...
    public boolean equals(Object o)
    {
        if (this == o) {
            ...
            RetentionLostSimStatFormulaQuota that = (RetentionLostSimStatFormulaQuota) o;
            return this.rowCount == that.rowCount && Arrays.equals(this.quotaType, that.quotaType) && Arrays.equals(this.quotaIndex, that.quotaIndex) && Arrays.equals(this.statTypes, that.statTypes) && Arrays.equals(this.table, that.table) && this.formula.equals(that.formula);
        }
        else {
            return false;
        }
    ...
    public int hashCode()
    {
        int result = Objects.hash(this.rowCount, this.formula, Arrays.hashCode(this.quotaType), Arrays.hashCode(this.quotaIndex));
    ...
    int estimateMemorySize()
    {
        ...
        return (int) ((long) INSTANCE_SIZE + SizeOf.sizeOf(this.quotaIndex) + SizeOf.sizeOf(this.quotaType) + SizeOf.sizeOf(this.statTypes) + SizeOf.sizeOf(this.table) + rowSize * (long) this.rowCount + (long) this.formula.length());
    }

    RetentionLostSimStatFormulaQuota compact()
    {
        if (this.rowCount < this.table.length) {
            ...
            return new RetentionLostSimStatFormulaQuota(this.rowCount, this.statTypes, compactTable, this.formula, this.quotaType, this.quotaIndex);
        }
        else {
            return this;
        }
